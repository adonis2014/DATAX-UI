...
import com.alibaba.otter.shared.communication.model.config.NotifyPipeLineEvent;
...
public class NodeTaskServiceImpl implements NodeTaskService, InitializingBean {
    ...
    public NodeTaskServiceImpl() {
        CommunicationRegistry.regist(ConfigEventType.notifyPipeline, this);
    }
    ...
    private void initNodeTask() {
        ...
        //改为直接从服务端management端获取Pipeline集合
        Object obj = nodeCommmunicationClient.callManager(event);
        if (obj != null) {
            List<Pipeline> pipelines = (List<Pipeline>) obj;
            for (Pipeline pipeline : pipelines) {
                // 排除已经分配过的task
                processNodeTask(pipeline);
            }
        }
    }

    private void processNodeTask(Pipeline pipeline) {
        //获取当前pipeline的nodetask 接点任务
        List<NodeTask> addTasks = parseNodeTask(pipeline);
        if (logger.isInfoEnabled()) {
            logger.info("##merge channel[{}] inc NodeTask {}", pipeline.getId(), printNodeTasks(addTasks));
        }
    ...
    private List<NodeTask> parseNodeTask(Pipeline pipeline) {
        List<NodeTask> tasks = new ArrayList<NodeTask>();
        Long nid = configClientService.currentNode().getId();
        //根据pipeline状态设置Task事件
        TaskEvent taksEvent = null;
        if (pipeline.getStatus().isStart()) {
            taksEvent = TaskEvent.CREATE;
        } else if (pipeline.getStatus().isStop()) {
            taksEvent = TaskEvent.DELETE;
        } else if (pipeline.getStatus().isPause()) {
        ...
        for (NodeTask task : allTasks) {
            Pipeline currpipeline = task.getPipeline();
            if (pipeline.getId()==currpipeline.getId()) {// 同一个pipeline
                boolean needAdd = false;
                NodeTask matchTask = getMatchTask(tasks, task);// 找到对应的匹配
                if (matchTask == null) {
                    matchTask = new NodeTask();
                    matchTask.setPipeline(currpipeline);
                    needAdd = true;
                }
     ...
     */
    protected synchronized boolean onNotifyChannel(NotifyPipeLineEvent event) {
        configClientService.createOrUpdatePipeline(event.getPipeline()); // 更新本地的config数据
        processNodeTask(event.getPipeline());

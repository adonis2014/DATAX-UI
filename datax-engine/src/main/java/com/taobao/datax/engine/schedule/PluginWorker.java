...
/**
 * {@link PluginWorker} represents executor of {@link Pluginable}.</br> DataX
 * {@link Engine} uses {@link PluginWorker} to load methods from each
 * {@link Pluginable} by reflections </br> and call each method later.
 * 
 * */
public abstract class PluginWorker {
	...
	public PluginWorker(PluginConf pluginConf, Class<?> myClass) {
		try {
			...
			this.setParam = myClass.getMethod("setParam", new Class[] { Class.forName("com.taobao.datax.common.plugin.PluginParam") });

			this.setMonitor = myClass.getMethod("setMonitor", new Class[] { Class.forName("com.taobao.datax.common.plugin.PluginMonitor") });
			this.getMonitor = myClass.getMethod("getMonitor", new Class[] {});

			this.init = myClass.getMethod("init", new Class[] {});

			this.split = myClass.getMethod("split", new Class[] { Class.forName("com.taobao.datax.common.plugin.PluginParam") });

			this.prepare = myClass.getMethod("prepare", new Class[] { Class.forName("com.taobao.datax.common.plugin.PluginParam") });

			this.post = myClass.getMethod("post", new Class[] { Class.forName("com.taobao.datax.common.plugin.PluginParam") });

			this.cleanup = myClass.getMethod("cleanup", new Class[] {});

			this.regMyMetaData = myClass.getMethod("setMyMetaData", new Class[] { Class.forName("com.taobao.datax.common.plugin.MetaData") });

			this.getMyMetaData = myClass.getMethod("getMyMetaData", new Class[] {});

			this.setOppositeMetaData = myClass.getMethod("setOppositeMetaData", new Class[] { Class.forName("com.taobao.datax.common.plugin.MetaData") });

			this.getOppositeMetaData = myClass.getMethod("getOppositeMetaData", new Class[] {});
	 ...
	 *            {@link PluginParam}
	 * 
	 * @return a list of sub-job {@link PluginParam}
	 * 
	 * @throws DataExchangeException
	 *
	 * */
	...
	public List<PluginParam> doSplit(PluginParam oParam) {
		try {
			return (List<PluginParam>) split.invoke(myObject, new Object[] { oParam });
		} catch (Exception e) {
			logger.error(ExceptionTracker.trace(e));
			throw new DataExchangeException(e.getCause());
		}

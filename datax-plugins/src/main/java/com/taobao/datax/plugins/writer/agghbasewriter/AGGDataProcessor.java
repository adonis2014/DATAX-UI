 ...
 * 
 */
public class AGGDataProcessor extends DataProcessor {
	...
	private Logger logger = Logger.getLogger(AGGDataProcessor.class);
	...
	public AGGDataProcessor(AGGHBaseProxy proxy) {
		this.proxy = proxy;
	}
...
//				logger.info("rowKeyColumns size:"+rowKeyColumns.size()+"  pkFields:"+pkFields.size());
				rowKeyColumns.clear();
				continue;
			}
...
//				logger.info("find chongfu row "+record.getAggRowNum()+" id:"+rowKey);
			}
	...
	public boolean validate(Line line) {
		...
		//判断主键是否有值，没有则忽略该记录
		for (DataProcessField field : fields) {
			if (line.checkAndGetField(field.getId())!=null){

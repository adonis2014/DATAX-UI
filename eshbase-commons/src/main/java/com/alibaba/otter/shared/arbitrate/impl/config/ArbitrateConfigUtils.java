...
public class ArbitrateConfigUtils {
    ...
    public static List<String> getServerAddrs() {
        Node node = ArbitrateConfigRegistry.getConfig().currentNode();
        if (node != null) {
            String addr = StringUtils.join(node.getCluster().getServerList(), ',');
    ...
    /**
     * 根据pipelineId获取task配置中定义的Project
     */
    public static Project getProject(Long pipelineId) {
        return ArbitrateConfigRegistry.getConfig().findProjectByPipelineId(pipelineId);
    }
    ...
    public static Project getProjectByProjectId(Long projectId) {
        return ArbitrateConfigRegistry.getConfig().findProject(projectId);
    }

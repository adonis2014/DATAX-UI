...
public class TerminMemoryArbitrateEvent implements TerminArbitrateEvent {
    ...
    public void single(TerminEventData data) {
        ...
        if (type.isNormal()) {
            ...
            Project channel = ArbitrateConfigUtils.getProject(data.getPipelineId());
            if (data.getType().isRollback()) {
                boolean paused = pipelineEvent.pause(channel.getId(), false);
                if (paused) {// 如果pause成功，则发送报警信息
                    warningTerminProcess.process(data);
                }
            } else if (data.getType().isShutdown()) {
                boolean shutdowned = pipelineEvent.stop(channel.getId(), false);
                // 发送报警信息
                if (shutdowned) {
                    warningTerminProcess.process(data);
                }
                ...
                boolean restarted = pipelineEvent.restart(channel.getId(), false);
                // 发送报警信息
                if (restarted) {
                    warningTerminProcess.process(data);
                }

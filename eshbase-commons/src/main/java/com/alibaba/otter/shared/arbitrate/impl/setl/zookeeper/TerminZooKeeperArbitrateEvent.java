...
public class TerminZooKeeperArbitrateEvent implements TerminArbitrateEvent {
    ...
    public void single(final TerminEventData data) {
        ...
        if (type.isNormal()) {
            ...
            Pipeline channel = ArbitrateConfigUtils.getPipeline(data.getPipelineId());

            if (data.getType().isRollback()) {
                boolean paused = pipelineEvent.pause(channel.getId());
                if (paused) {// 如果pause成功，则发送报警信息
                    warningTerminProcess.process(data);
                }
            } else if (data.getType().isShutdown()) {
                boolean shutdowned = pipelineEvent.stop(channel.getId());
                // 发送报警信息
                if (shutdowned) {
                    warningTerminProcess.process(data);
                }
                ...
                boolean restarted = pipelineEvent.restart(channel.getId());
                // 发送报警信息
                if (restarted) {
                    warningTerminProcess.process(data);
                }

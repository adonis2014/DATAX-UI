...
public class PrestoDBMetaReverseImpl extends JDBCMetaReverse {
	...
	public DataSource reverseMeta(DataSource datasource, List<MetaProperty> mproes, User cuser) {
		...
		try {
			...
			// 获取所有数据库
			List<Map<String, String>> dbs = getPrestoSqlQuery(conn, "show schemas", null);
			for (Map<String, String> row : dbs) {
				String dbname = row.get("Schema");
				...
				db.setRemark(dbname);
				// 获取表信息
				List<Map<String, String>> tabs = getPrestoSqlQuery(conn, "show tables", dbname);
				for (Map<String, String> tab : tabs) {
					String tableName = tab.get("Table");
					if (StringUtils.isNotEmpty(tableName)) {
						...
						List<Map<String, String>> cols = getPrestoSqlQuery(conn, "describe " + tableName, null);
						for (Map<String, String> colMap : cols) {
							String sqlColName = colMap.get("Column");
							if (StringUtils.isNotEmpty(sqlColName) && !StringUtils.startsWith(sqlColName, "#")) {
								...
								col.setType(colMap.get("Type"));
								table.addColumn(col);
							}
						}
						List<Map<String, String>> rows = getPrestoSqlQuery(conn, "select * from " + tableName+" limit 10 ", null);
						table.setSampleRows(mapper.writeValueAsString(rows));
	...
	private List<Map<String, String>> getPrestoSqlQuery(Connection conn, String sql, String dbname) {

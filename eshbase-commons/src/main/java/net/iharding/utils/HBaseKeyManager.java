...
//		}
	...
	public long initStartNum()  {
		try {
			if (keyTable==null){
				keyTable = HBaseUtils.getTable(Constants.SYS_TABLE_INCREMENT_ENTITY_CODE);
			}
			...
			if (ur != null) {
				return HBStringUtils.getIntValueFromResult(ur,family,Constants.SYSINCREMENT_FIELD_MAXID);
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public void updateStartNum(long intervalMax)  {
		try {
			...
			p.addColumn(Bytes.toBytes(Constants.DEFAULT_COLUMN_FAMILY), Bytes.toBytes(Constants.SYSINCREMENT_FIELD_MAXID), Bytes.toBytes(String.valueOf(intervalMax)));
			keyTable.put(p);
		} catch (IOException e) {
			e.printStackTrace();
		}

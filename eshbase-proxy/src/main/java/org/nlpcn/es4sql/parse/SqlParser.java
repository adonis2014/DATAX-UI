	...
	private void explanCond(String opear, SQLExpr expr, Where where) throws SqlParseException {
        if (expr instanceof SQLBinaryOpExpr) {
            ...
            if(SpatialParamsFactory.isAllowedMethod(methodName)){
                ...
                where.addWhere(condition);
            }else if (methodName.toLowerCase().equals("parent")){
            	ParentType parentType = new ParentType();

                if(!parentType.tryFillFromExpr(expr)){
                    throw new SqlParseException("could not fill children from expr:" + expr);
                }

                Condition condition = new Condition(CONN.valueOf(opear), parentType.parentType, methodName.toUpperCase(), parentType.where);

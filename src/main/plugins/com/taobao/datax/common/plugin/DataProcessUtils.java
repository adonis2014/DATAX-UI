	...
	public static void aggValue(AggregationRecord record, Map<String, String> curRow, DataProcessField field) {
		...
		if (ETLStringUtils.isEmpty(field.getFunctionName()) && ETLStringUtils.isEmpty(field.getScript())) {
			...
			if (ETLStringUtils.isNotEmpty(field.getScript())) {
				...
				if ("primaryKey".equalsIgnoreCase(field.getFunctionName())) {//直接读取主键
					record.putValue(field.getColumnName(),record.getRowKey());
				}else if ("rowNum".equalsIgnoreCase(field.getFunctionName())) {//直接读取主键
					record.putValue(field.getColumnName(),record.getRowNum());
				}else if ("aggRowNum".equalsIgnoreCase(field.getFunctionName())) {//直接读取主键
					record.putValue(field.getColumnName(),record.getAggRowNum());
				}else if ("count".equalsIgnoreCase(field.getFunctionName())) {// 计数
					record.putValue(field.getColumnName(), "" + (NumberUtils.toInt(record.getValue(field.getColumnName()).toString(), 0) + 1));
				}else if ("sum".equalsIgnoreCase(field.getFunctionName())) {// 求和
				...
				}else if ("avg".equalsIgnoreCase(field.getFunctionName())) {// 求平均值
					record.putValue(field.getColumnName(), "" + (NumberUtils.toFloat(record.getValue(field.getColumnName()).toString(), 0) * (record.getAggRowNum() - 1) + NumberUtils.toFloat(fieldValue))
							/ record.getAggRowNum());
				}else if ("max".equalsIgnoreCase(field.getFunctionName())) {// 最大值
				...
				} else if ("min".equalsIgnoreCase(field.getFunctionName())) {// 最小值
					String rvalue = record.getValue(field.getColumnName()).toString();
					if (NumberUtils.isNumber(rvalue)) {
						if (NumberUtils.toFloat(rvalue) > NumberUtils.toFloat(fieldValue)) {
							record.putValue(field.getColumnName(), fieldValue);
						...
						if (rvalue.compareTo(fieldValue) == 1) {
							record.putValue(field.getColumnName(), fieldValue);
						}
					}
				} else if ("firstValue".equalsIgnoreCase(field.getFunctionName())) {// 第一个值
					if (record.getRowNum() == 1) {
						record.putValue(field.getColumnName(), fieldValue);
					}
				} else if ("lastValue".equalsIgnoreCase(field.getFunctionName())) {// 最后一个值
					record.putValue(field.getColumnName(), fieldValue);
				} else if ("firstNoNull".equalsIgnoreCase(field.getFunctionName())) {// 第一个非null值
					if (ETLStringUtils.isEmpty(record.getValue(field.getColumnName()).toString()) && ETLStringUtils.isNotEmpty(fieldValue)) {
						record.putValue(field.getColumnName(), fieldValue);
					}
				} else if ("lastNoNull".equalsIgnoreCase(field.getFunctionName())) {// 最后一个非null值
					if (ETLStringUtils.isNotEmpty(fieldValue)) {
						record.putValue(field.getColumnName(), fieldValue);
					}
				} else if ("joinAsString".equalsIgnoreCase(field.getFunctionName())) {// 合并字符串，间隔
					if (record.getRowNum() == 1) {
						record.putValue(field.getColumnName(), fieldValue);
					} else {
						record.putValue(field.getColumnName(), record.getValue(field.getColumnName()) + "," + fieldValue);
					}
				} else if ("joinAsList".equalsIgnoreCase(field.getFunctionName())) {// 合并字符串为LIST集合
					if (record.getRowNum() == 1) {
				...
				} else if ("joinAsSet".equalsIgnoreCase(field.getFunctionName())) {// 合并字符串为Set去重集合
					if (record.getRowNum() == 1) {
				...
				}else if ("joinAsListMap".equalsIgnoreCase(field.getFunctionName())) {// 合并字符串为Set去重集合
					if (record.getRowNum() == 1) {
				...
				}else if ("joinAsRangeMap".equalsIgnoreCase(field.getFunctionName())) {// 合并统计按照参数范围合并
					if (record.getRowNum() == 1) {
						List<Map<String,Object>> jset = new ArrayList<Map<String,Object>>();
						Map<String,Object> row=new ConcurrentHashMap<String, Object>();
		...
		for (DataProcessField field : fields) {
			...
			if (ETLStringUtils.isEmpty(field.getFunctionName()) && ETLStringUtils.isEmpty(field.getScript())) {
				...
				if (ETLStringUtils.isNotEmpty(field.getScript())) {
					...
					if ("primaryKey".equalsIgnoreCase(field.getFunctionName())) {//直接读取主键
					...
					}else if ("rowNum".equalsIgnoreCase(field.getFunctionName())) {
						curRecord.putValue(field.getColumnName(),curRecord.getRowNum());
					}else if ("aggRowNum".equalsIgnoreCase(field.getFunctionName())) {//直接读取
						...
						curRecord.putValue(field.getColumnName(),
						...
						if (NumberUtils.isNumber(curFieldValue.toString())) {
							if (NumberUtils.toFloat(curFieldValue.toString()) < NumberUtils.toFloat(fieldValue.toString())) {
								curRecord.putValue(field.getColumnName(), fieldValue);
							}
						} else {
							if (curFieldValue.toString().compareTo(fieldValue.toString()) == 0) {
								curRecord.putValue(field.getColumnName(), fieldValue);
							}
						}
					} else if ("min".equalsIgnoreCase(field.getFunctionName())) {// 最小值
						if (NumberUtils.isNumber(curFieldValue.toString())) {
							...
							if (curFieldValue.toString().compareTo(fieldValue.toString()) == 1) {
								curRecord.putValue(field.getColumnName(), fieldValue);
							}
						}
					} else if ("firstValue".equalsIgnoreCase(field.getFunctionName())) {// 第一个值,取原始值
						curRecord.putValue(field.getColumnName(), fieldValue);
					} else if ("lastValue".equalsIgnoreCase(field.getFunctionName())) {// 最后一个值,取当前记录值
//						curRecord.putValue(field.getColumnName(), fieldValue);
					} else if ("firstNoNull".equalsIgnoreCase(field.getFunctionName())) {// 第一个非null值，取原始值
					...
					} else if ("joinString".equalsIgnoreCase(field.getFunctionName())) {// 合并字符串，间隔
						if (curRecord.getRowNum() == 1) {
							curRecord.putValue(field.getColumnName(), fieldValue);
						} else {
							if (StringUtils.isNotEmpty(curFieldValue.toString())){
						...
						if (fieldValue!=null){
							...
							try {
								if (ETLStringUtils.isNotEmpty(fieldValue))oriList = mapper.readValue(fieldValue, new TypeReference<List<String>>(){});
							} catch (IOException e) {
								e.printStackTrace();
							}
							if (oriList!=null)jlist.addAll(oriList);
						...
						if (fieldValue!=null) {
							...
							try {
								if (ETLStringUtils.isNotEmpty(fieldValue))oriset = mapper.readValue(fieldValue, new TypeReference<Set<String>>(){});
							}  catch (IOException e) {
								e.printStackTrace();
							} 
							if (oriset!=null)jset.addAll(oriset);
							curRecord.putValue(field.getColumnName(), jset);
						}
					}else if ("joinAsListMap".equalsIgnoreCase(field.getFunctionName())) {// 合并字符串为map并统计
						if (fieldValue!=null) {
					...
					}else if ("joinAsRangeMap".equalsIgnoreCase(field.getFunctionName())) {// 合并字符串按照参数范围合并
						if (fieldValue!=null) {
							List<Map<String,Object>> jset =(List<Map<String,Object>>)curFieldValue;
							List<Map<String, Object>> oriJset=null;
							try {
